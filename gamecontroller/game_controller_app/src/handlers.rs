...
async fn launch(settings: LaunchSettings, window: WebviewWindow, app: AppHandle) {
    ...
    let window_for_goal = window.clone();
    ...
    let send_goal_footage = {
        move |payload: game_controller_runtime::GoalFootagePayload| {
            window_for_goal.emit("goal-footage", payload).map_err(|e| anyhow!(e))
        }
    };
    ...
    match start_runtime(
        ...
        Some(Box::new(send_goal_footage)),
    )
    .await
