...
use game_controller_core::{
    action::{Action, VAction},
    actions::{Goal, TeamMessage},
    log::{
        LogEntry, LoggedGoalFootage, LoggedMetadata, LoggedMonitorRequest, LoggedStatusMessage, LoggedTeamMessage,
        TimestampedLogEntry,
    },
...
use game_controller_net::{
    ActionCommandReceiver, ControlMessageSender, Event, GoalFootageReceiver, MonitorRequestReceiver,
...
/// Payload for goal footage events sent to the frontend.
#[derive(Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct GoalFootagePayload {
    pub host: String,
    pub team_id: u8,
    pub seq: u16,
    pub timestamp: u32,
    /// JPEG-encoded frames as base64-ready byte vectors
...
async fn start_network(
    ...
    let monitor_request_receiver = MonitorRequestReceiver::new(local_address, event_sender.clone())
        .await
        .context("could not create monitor request receiver")?;
    join_set.spawn(async move { monitor_request_receiver.run().await.unwrap() });

    let goal_footage_receiver = GoalFootageReceiver::new(local_address, event_sender.clone())
        .await
        .context("could not create goal footage receiver")?;
    join_set.spawn(async move { goal_footage_receiver.run().await.unwrap() });
...
async fn event_loop(
    ...
    send_goal_footage: Option<Box<dyn Fn(GoalFootagePayload) -> Result<()> + Send>>,
) -> Result<()> {
        ...
        select! {
            ...
            event = event_receiver.recv() => {
                ...
                match event {
                    ...
                    Some(Event::GoalFootage { host, team_id, seq, timestamp, frames }) => {
                        let frame_count = frames.len() as u8;
                        game_controller.log_now(LogEntry::GoalFootage(LoggedGoalFootage {
                            host,
                            team_id,
                            seq: seq as u32,
                            timestamp,
                        ...
                        // Auto-apply Goal action (no review popup)
                        let side = if team_id == 1 { Side::Home } else { Side::Away };
                        let action = VAction::Goal(Goal { side });
                        if game_controller.apply(action, ActionSource::Network) {
                            println!("[AUTO] Goal for {:?} from Jetson {}", side, host);
                        } else {
                            println!("[AUTO] Goal for {:?} from {} ILLEGAL",
                                     side, host);
                        }
                    },
                    _ => {},
...
pub async fn start_runtime(
    ...
    // TCP command listener for external actions (Python review popup)
    {
        let action_sender = action_sender.clone();
        let local_addr = network_interface.address;
        runtime_join_set.spawn(async move {
            ActionCommandReceiver::new(local_addr, action_sender)
                .await?
                .run()
                .await
        });
